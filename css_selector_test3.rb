...
elements.each do |element|
    ...
    begin
        ...
        # # review = Review.new(content.text,local_guide.text.scan(/(\d+)\ 件のレビュー/)[0].to_s.to_i, star_score)
        # reviews.push(review)
        ...
        # review = Review.new(content.text,local_guide.text.match(/(\d+)\ 件のレビュー/)[0].to_i, star_score)
        # # review = Review.new(content.text,local_guide.text.scan(/(\d+)\ 件のレビュー/)[0].to_s.to_i, star_score)
        # reviews.push(review)
        # # puts local_guide_info.text
        # # puts content.text
